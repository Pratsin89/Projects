...
def update_scatter_chart(entered_site, payload_range):
    ...
    PAYLOAD_COLUMN = 'Payload Mass (kg)' # Use the correct column name

    # --- 1. Filter by Payload Mass Range (Applies to ALL cases) ---
    filtered_df = filtered_df[
        (filtered_df[PAYLOAD_COLUMN] >= low) & 
        (filtered_df[PAYLOAD_COLUMN] <= high)
    ]

    # --- 2. If-Else: Filter by Launch Site and Plot ---
    if entered_site == 'ALL':
        ...
        # Case 2: A specific launch site is selected
        # Filter the DataFrame further by the selected launch site
        final_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        title = f'Payload vs. Outcome for Site {entered_site} (Payload: {low}-{high} kg)'
        
    # --- 3. Create the Scatter Plot (Same logic for both cases) ---
    fig = px.scatter(
        final_df, 
        x=PAYLOAD_COLUMN, 
        y='class', 
        # Color the points based on the Booster Version Category as required
        color='Booster Version Category', 
        title=title,
        labels={PAYLOAD_COLUMN: 'Payload Mass (kg)', 'class': 'Mission Outcome (0=Failure, 1=Success)'},
        hover_data=['Reused', 'Launch Site']
    )
    
    # Optional y-axis customization
